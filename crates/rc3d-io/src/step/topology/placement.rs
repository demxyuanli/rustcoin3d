...
pub fn resolve_point(point_id: u64, entities: &EntityIndex) -> Option<PVec3> {
    ...
    Some(PVec3::new(
        ...
        coords[2].as_real()? as Real,
    ))
}

/// AXIS1_PLACEMENT: rotation axis as (origin, unit direction).
pub fn resolve_axis1_placement(place_id: u64, entities: &EntityIndex) -> Option<(PVec3, PVec3)> {
    ...
    let axis = resolve_direction(axis_id, entities).unwrap_or(PVec3::Z);
    Some((origin, axis.normalize()))
}

/// Sweep/revolution axis from AXIS1_PLACEMENT or AXIS2_PLACEMENT_3D (Z axis).
pub fn resolve_sweep_axis(place_id: u64, entities: &EntityIndex) -> Option<(PVec3, PVec3)> {
...
pub fn resolve_placement(point_id: u64, entities: &EntityIndex) -> Option<(PVec3, PVec3, PVec3)> {
    let record = entities.get(&point_id)?;
    ...
    let origin = resolve_point(origin_id, entities)?;
    let axis = resolve_direction(axis_id, entities).unwrap_or(PVec3::Z);
    let ref_dir = refdir_id.and_then(|id| resolve_direction(id, entities)).unwrap_or(PVec3::X);
...
pub fn resolve_direction(dir_id: u64, entities: &EntityIndex) -> Option<PVec3> {
...
pub fn resolve_direction_public(dir_id: u64, entities: &EntityIndex) -> Option<PVec3> {
