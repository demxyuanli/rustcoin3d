...
use crate::gizmo_bind::{scene_pick_matrices, viewport_pick_matrices};
...
impl EventRouteOpts {
    /// Middle-button orbit, no click-pick (editor / annotation drag hosts).
    pub fn editor() -> Self {
        Self {
            left_orbit: false,
            pick_on_click: false,
        ...
        if !result.overlay_consumed {
            let (lx, ly, vw, vh, view, proj) = if pointer_event {
                self.pointer_pick_frame(cx, cy, ww, wh)
                    .unwrap_or_else(|| {
                        let (v, p) = scene_pick_matrices(self);
                        (cx, cy, ww.max(1) as f32, wh.max(1) as f32, v, p)
                    })
            } else {
                let (v, p) = scene_pick_matrices(self);
                (cx, cy, ww.max(1) as f32, wh.max(1) as f32, v, p)
