...
fn main() {
    ...
    let blend = BlendNode::Blend {
        left: Box::new(BlendNode::Clip {
            clip: idle,
            speed: 1.0,
            start_time: 0.0,
        }),
        right: Box::new(BlendNode::Clip {
            clip: walk,
            speed: 1.0,
            start_time: 0.0,
        }),
    ...
    if let Some(_pose) = blend.sample(0.0, &skeleton) {
        println!("Blend tree sampled successfully at time 0");
    }
