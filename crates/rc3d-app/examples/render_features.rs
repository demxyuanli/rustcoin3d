...
impl Engine for RenderFeaturesPanelEngine {
    fn evaluate(&mut self, graph: &mut SceneGraph, time: f64) {
        ...
        let mut weights = [s.channel_a_weight, s.channel_b_weight, s.channel_c_weight];
        ...
        for (i, &lid) in self.demo_light_ids.iter().enumerate() {
            if let Some(entry) = graph.get_mut(lid) {
                if let NodeData::PointLight(light) = &mut entry.data {
                    ...
                    if self.idle_mode {
                        light.location = Vec3::new(-2.0 + i as f32 * 2.0, 1.5, 2.0);
                    } else {
                        let offset = i as f32 * 2.1;
                        light.location = Vec3::new(
                            (self.phase * (0.6 + i as f32 * 0.25) + offset).sin() * 2.0,
                            1.5 + wobble,
                            (self.phase * (0.4 + i as f32 * 0.15) + offset).cos() * 2.0,
                        );
                    }
                }
            }
...
fn main() {
    ...
    let event_loop =
        winit::event_loop::EventLoop::new().expect("failed to create render_features event loop");
    ...
    if !no_panel {
        ...
        app = app
            .with_panel_overlay_text_hook(move || {
                ...
                build_render_features_panel_overlay_with_selection(&panel_for_text, sel)
            })
            .with_panel_overlay_key_hook(move |key| {
                apply_render_features_panel_key(&panel_for_key, &selected_for_key, key)
            })
            .with_panel_overlay_mouse_hook(move |x, y, w, h| {
                apply_render_features_panel_mouse(&panel_for_mouse, &selected_for_mouse, x, y, w, h)
            });
    }
    ...
    println!("  Shared panel binding: model visibility, debug overlay toggle state, channel enable/mix -> demo light intensity, global rate -> light motion speed");
...
fn apply_render_features_panel_key(
    ...
    match key {
        ...
        KeyCode::ArrowLeft => match *sel {
            5 => s.channel_a_weight = (s.channel_a_weight - 0.05).clamp(0.0, 1.0),
            6 => s.channel_b_weight = (s.channel_b_weight - 0.05).clamp(0.0, 1.0),
            7 => s.channel_c_weight = (s.channel_c_weight - 0.05).clamp(0.0, 1.0),
            8 => s.time_scale = (s.time_scale - 0.1).max(0.0),
            _ => {}
        },
        KeyCode::ArrowRight => match *sel {
            5 => s.channel_a_weight = (s.channel_a_weight + 0.05).clamp(0.0, 1.0),
            6 => s.channel_b_weight = (s.channel_b_weight + 0.05).clamp(0.0, 1.0),
            7 => s.channel_c_weight = (s.channel_c_weight + 0.05).clamp(0.0, 1.0),
...
fn print_render_features_help() {
    ...
    println!("Usage: cargo run -p rc3d-app --example render_features -- <file.gltf|file.glb|file.obj|file.stl|file.iv> [--no-panel] [--idle] [--adaptive-quality=off|on|auto-idle-lock]");
    println!("Controls:");
    ...
    println!(
        "  Shared panel: visibility, channel enable, update control, channel mix, global rate"
    );
}
...
fn ensure_scene_setup(mut graph: SceneGraph) -> SceneGraph {
    ...
    let has_light = has_node_type(&graph, |d| {
        matches!(
            d,
            NodeData::DirectionalLight(_) | NodeData::PointLight(_) | NodeData::SpotLight(_)
        )
    });
    ...
    let root = graph
        ...
        .first()
        .copied()
        .unwrap_or_else(|| graph.add_root(NodeData::Separator(SeparatorNode)));
...
fn attach_panel_demo_lights(graph: &mut SceneGraph) -> ([NodeId; 3], [f32; 3]) {
    let root = graph
        .roots()
        .first()
        .copied()
...
fn attach_render_debug_overlay(graph: &mut SceneGraph) -> NodeId {
    let root = graph
        ...
        .copied()
        .unwrap_or_else(|| graph.add_root(NodeData::Separator(SeparatorNode)));
    let overlay = graph.add_child(root, NodeData::Transform(TransformNode::default()));
    let grid_size = 6;
    let spacing = 1.0f32;
    ...
    for i in -grid_size..=grid_size {
        ...
        let line = graph.add_child(overlay, NodeData::Separator(SeparatorNode));
        graph.add_child(
            line,
            NodeData::Transform(TransformNode {
                translation: Vec3::new(0.0, 0.01, z),
                scale: Vec3::new(grid_size as f32 * spacing * 2.0, 0.01, 0.01),
                ..Default::default()
            }),
        );
        graph.add_child(
    ...
    for i in -grid_size..=grid_size {
        ...
        graph.add_child(
            ...
            NodeData::Transform(TransformNode {
                ...
                scale: Vec3::new(0.01, 0.01, grid_size as f32 * spacing * 2.0),
                ..Default::default()
            }),
        );
        graph.add_child(
    ...
    let axis_sep = graph.add_child(overlay, NodeData::Separator(SeparatorNode));
    graph.add_child(
        axis_sep,
        NodeData::Transform(TransformNode {
