...
fn main() {
    ...
    g.add_child(root, NodeData::Material(MaterialNode { base_color: Vec3::new(0.5, 0.5, 0.5), roughness: 0.4, ..Default::default() }));
    // Large flat ground to project decal onto
    let ground = g.add_child(root, NodeData::Transform(TransformNode::from_translation(Vec3::new(0.0, -1.0, 0.0))));
    g.add_child(ground, NodeData::Cube(CubeNode { width: 5.0, height: 0.1, depth: 5.0 }));
    // Decal projected from above onto the ground
    g.add_child(root, NodeData::Decal(DecalNode { position: Vec3::new(0.0, 0.5, 0.0), direction: Vec3::NEG_Y, size: [2.0, 2.0], texture_path: "decal.png".into(), color: [1.0, 0.5, 0.0, 0.8], opacity: 0.8 }));
    println!("Decal shader at shaders/decal_project.wgsl — ensure_decal_pass() initializes pipeline");
