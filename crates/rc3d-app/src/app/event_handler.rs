...
pub(crate) fn window_event(
    ...
    match &event {
                ...
                match key {
                    winit::keyboard::PhysicalKey::Code(winit::keyboard::KeyCode::KeyW) => {
                        if app
                            ...
                            .unwrap_or(false)
                        {
                            if let Some(ref mut cc) = app.state.camera_controller {
                                cc.walk(0.1, 0.0, 0.0, 0.15);
                            }
                        ...
                        if app.input.ctrl_pressed {
                            ...
                            for id in selected {
                                let _ = app
                                    .state
                                    .editor_commands
                                    .push_back(EditorCommand::DuplicateNode(id));
                            }
                        } else if let Some(ref mut cc) = app.state.camera_controller {
                            if cc.walk_mode {
                                cc.walk(0.0, 0.1, 0.0, 0.15);
                    ...
                    winit::keyboard::PhysicalKey::Code(
                        ...
                        for id in selected {
                            let _ = app
                                .state
                                .editor_commands
                                .push_back(EditorCommand::DeleteNode(id));
                        }
                    }
        ...
        WindowEvent::MouseInput { state, button, .. } => {
            ...
            if !app.editor.measurement_mode
                && !app.should_block_handle_event_pointer_dispatch(false)
            {
        ...
        WindowEvent::RedrawRequested => {
            ...
            if let (Some(renderer), Some(window)) = (&mut app.state.renderer, &app.state.window) {
                ...
                if renderer.has_lod_nodes() {
                    ...
                    for &r in &roots_lod {
                        total_lod += update_all_lod_nodes(
                            &mut app.state.world.graph,
                            r,
                            app.state.last_camera_eye,
                        );
                    }
                ...
                if can_skip && !controller_camera_moved {
                    ...
                    app.state.world.collector.material_library =
                        Some(app.state.world.materials.clone());
                    app.state
                        .world
                        .collector
                        .set_hidden_nodes(&app.state.hidden_nodes);

                    let _t0 = std::time::Instant::now();
                    app.state
                        .world
                        .collector
                        .reserve_draw_calls(app.state.world.cached_draw_calls.len());
                    for &root in app.state.world.graph.roots() {
                        app.state
                            .world
                            .collector
                            .traverse(&app.state.world.graph, root);
                    }
                    let traversal_ms = _t0.elapsed().as_secs_f64() * 1000.0;
                    if traversal_ms > 5.0 {
                        log::info!(
                            "Scene traversal: {:.2}ms ({} nodes, {} dirty roots)",
                            traversal_ms,
                ...
                if !can_skip && !camera_only {
                    ...
                    renderer.set_effect_commands(std::mem::take(
                        &mut app.state.world.collector.effect_commands,
                    ));
                ...
                let cam_interacting =
                    ...
                    }) || app.state.viewport_cameras.active().map_or(false, |vc| {
                ...
                if !app.state.world.collector.draw_calls.is_empty() {
                    ...
                    let adaptive_control = if large_scene {
                        ...
                        match app.state.adaptive_quality_mode {
                    ...
                    app.state
                        .fps_tracker
                        .maybe_log(&app.state.last_render_stats, &quality);
    ...
    let block_orbit = app.editor.gizmo_dragging
        || app.editor.box_select_drag
        || app.editor.view_split_drag.is_some();
