...
fn main() {
    ...
    println!("Usage: cargo run -p rc3d-examples --example profile_viewer");
    ...
    run_example_with_hooks("Profile Viewer", move |engine| {
        ...
        engine.hud_text_hook = Some(Box::new(move || {
            ...
            lines.push("".to_string());
            lines.push(
                "[1-4] Complexity  [P] Print report  [Esc] Clear selection"
                    .to_string(),
            );
            lines.join("\n")
        }));
        engine.panel_overlay_key_hook = Some(Box::new({
            ...
            move |key: PhysicalKey| -> bool {
                let new_count = match key {
                    PhysicalKey::Code(KeyCode::Digit1) => Some(10),
                    PhysicalKey::Code(KeyCode::Digit2) => Some(100),
                    PhysicalKey::Code(KeyCode::Digit3) => Some(500),
                    PhysicalKey::Code(KeyCode::Digit4) => Some(2000),
                    _ => None,
                };
                if let Some(count) = new_count {
                    if let Ok(mut s) = state.lock() {
                        ...
                        println!(
                            "Switching to {} objects — restart example to apply",
                            count
                        );
