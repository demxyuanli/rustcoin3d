...
pub(crate) fn wire_native_v_span(samples: &[(f32, f32)]) -> (f32, f32) {
    let mut lo = f32::MAX;
    let mut hi = f32::MIN;
    for &(_, v) in samples {
        lo = lo.min(v);
...
pub(crate) fn nearest_periodic(value: f32, anchor: f32, period: f32) -> f32 {
    if period <= 0.0 {
        return value;
    }
...
pub(crate) fn normalized_v_bounds_for_period(uv0: &[(f32, f32)], uv1: &[(f32, f32)], period: f32) -> Option<(f32, f32)> {
    if period <= 0.0 {
        return None;
    }
...
pub(crate) fn u_at_v_on_wire(samples: &[(f32, f32)], v: f32) -> f32 {
    if samples.is_empty() {
        return 0.0;
    }
    if samples.len() == 1 {
...
pub(crate) fn v_at_u_on_wire(samples: &[(f32, f32)], u: f32) -> f32 {
    if samples.is_empty() {
        return 0.0;
    }
    if samples.len() == 1 {
...
pub(crate) fn wire_native_uv_spans(samples: &[(f32, f32)]) -> (f32, f32) {
    if samples.is_empty() {
        return (0.0, 0.0);
    }
...
pub(crate) fn revolution_wire_is_generatrix_profile(samples: &[(f32, f32)]) -> bool {
    let (du, dv) = wire_native_uv_spans(samples);
    du > dv * 2.0 && du > 1e-4
}

pub(crate) fn uv_mindiff(u: f32, u0: f32, period: f32) -> f32 {
    (-4..=4)
...
pub(crate) fn resample_uv_polyline(uvs: &[(f32, f32)], samples: usize) -> Vec<(f32, f32)> {
