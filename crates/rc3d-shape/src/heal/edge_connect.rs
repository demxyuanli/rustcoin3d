...
pub fn connect_shell_edges(reg: &mut BRepStore, shell_key: crate::topo::ShellKey, tolerance: Real) -> usize {
    ...
    let mut edge_endpoints: Vec<(EdgeKey, FaceKey, PVec3, PVec3)> = Vec::new();
    for &(fk, _) in &shell.faces {
        ...
        for wk in wire_keys {
            ...
            for &(ek, _) in &wire.edges {
                ...
                let lo = reg.vertices.get(edge.v_low).map(|v| v.position).unwrap_or(PVec3::ZERO);
                let hi = reg.vertices.get(edge.v_high).map(|v| v.position).unwrap_or(PVec3::ZERO);
...
fn quantize(p: PVec3, tol: Real) -> [i32; 3] {
...
mod tests {
    ...
    fn merge_coincident_vertices() {
        ...
        let f1 = reg.faces.insert(BRepFace {
            surface: SurfaceGeom::Plane { origin: PVec3::new(10.0, 0.0, 0.0), normal: PVec3::Z, u_dir: PVec3::X },
        ...
        let v1b = reg.vertices.insert(BRepVertex { position: PVec3::new(5.0, 1e-6, 0.0), tolerance: 1e-4 });
        let v2 = reg.find_or_add_vertex(PVec3::new(10.0, 0.0, 0.0), 1e-4);

        let pc = Curve2d::Line { origin: (0.0, 0.0), direction: (1.0, 0.0) };
        let c = CurveGeom::Line { origin: PVec3::ZERO, direction: PVec3::X };
