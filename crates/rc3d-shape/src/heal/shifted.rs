...
fn shift_pcurve(
    pc: &crate::geom::CurveGeom,
    du: f32,
    dv: f32,
) -> crate::geom::CurveGeom {
...
mod tests {
    ...
    fn test_shifted_cylinder_pcurve() {
        ...
        let e1 = reg.edges.insert(crate::topo::BRepEdge {
            v_low: v0, v_high: v1, curve: curve_3d.clone(), tolerance: 1e-4,
            pcurves: std::collections::HashMap::from([(fk, pc_normal.clone())]),
        });
        let e2 = reg.edges.insert(crate::topo::BRepEdge {
            v_low: v0, v_high: v1, curve: curve_3d.clone(), tolerance: 1e-4,
            pcurves: std::collections::HashMap::from([(fk, pc_normal.clone())]),
        });
        let e3 = reg.edges.insert(crate::topo::BRepEdge {
            v_low: v0, v_high: v1, curve: curve_3d.clone(), tolerance: 1e-4,
            pcurves: std::collections::HashMap::from([(fk, pc_normal)]),
        });
        let e4 = reg.edges.insert(crate::topo::BRepEdge {
