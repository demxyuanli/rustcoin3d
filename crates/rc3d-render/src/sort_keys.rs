...
pub fn vec4_array_sort_key(arr: [[f32; 4]; MAX_LIGHTS]) -> [[u32; 4]; MAX_LIGHTS] {
    let mut out = [[0u32; 4]; MAX_LIGHTS];
    for (i, c) in arr.iter().enumerate() {
        out[i] = color_sort_key(*c);
    }
    out
}
...
pub fn light_sort_key(dc: &DrawCall) -> (
    [[u32; 4]; MAX_LIGHTS], [[u32; 4]; MAX_LIGHTS], [[u32; 4]; MAX_LIGHTS],
    [[u32; 4]; MAX_LIGHTS], [[u32; 4]; MAX_LIGHTS],
