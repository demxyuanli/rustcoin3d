...
| **meshopt LOD 生成** + 渐进式流式加载 | 已实现 | HOOPS LOD 更灵活（per-segment tessellation） |
...
|------|-------------|---------|---------|
| Tier 0 编辑器 | 多视口布局、活动视口、剖分可调与分割条拖拽、Gizmo 库+应用内拖拽与绘制 | 场景图全键盘路由、指针路由与示例对齐 | 与所有示例统一 legacy 路径 |
| Tier 1 工业 | 透明排序、stencil 选择/包围盒、ScenePath/Search、剖面节点+GPU 裁剪、框选（frustum） | 剖面拖动手柄、OIT、离屏与主路径合一 | 套索、X-ray 全场景、帽面/剖面线 |
| Tier 2 场景图 | Switch/MultiCopy/LOD 节点、Text2/3、EventCallback、结构化拾取 | Field 描述符、面索引拾取、传感器 API | 双向 field 图、全量 SoEngine 族 |
...
| # | 差距 | 状态 (2026) | 说明 |
|---|------|-------------|------|
| T0-1 | **Manipulator/Gizmo** | Partial | `rc3d-gizmo` + `rc3d-app` 集成交互与线框绘制；非场景图 `TransformManip` 节点 |
| T0-2 | **多视口** | Partial | `ViewportLayout` Quad/分屏、持久化 split、视口里分割条拖拽与加粗分割线 |
| T0-3 | **场景图事件路由** | Partial | `HandleEventAction`：键盘/滚轮非 pick 全树；指针/滚轮 view·proj 取光标下视口 |
| T0-4 | **相机与视口** | Partial | `ViewportCameraSet`、滚轮命中视口缩放；中键旋转/右键平移锁定按下视口至松开、`Event::Scroll` 与 pick 一致；拟合、正交/透视 |

| # | 差距 | 状态 (2026) | 说明 |
|---|------|-------------|------|
| T1-1 | **透明排序** | Partial | 深度排序透明 pass + `opacity`；无 WBOIT |
...
| T1-5 | **LOD 场景图** | Partial | `Lod` 节点 + 距离自动切层 |
...
- **Phase 0**：Gizmo/多视口/事件/视口相机 — 核心已具备，持续打磨输入一致性与「节点级」操纵器。
- **Phase 1**：透明/剖面/高亮/LOD/Path/框选/拟合 — 大项已 Partial；OIT、帽面、X-ray 为后续。
...
- [x] **多视口** — `Viewport`、Quad、活动边框、split fractions、可拖拽分割条（分割线可视加粗）
- [x] **HandleEventAction** + `EventCallback` + 非鼠标遍历；编辑器指针与滚轮经光标命中视口的 view·proj 派发
- [x] **视口级相机** — `ViewportCameraSet`、per-viewport controller、拟合选中

### Phase 1

- [x] **透明** — depth-sorted + `opacity`（OIT 远期）
...
SoDragger / SoManip               rc3d-gizmo (库) + 应用层绑定
...
SoEngine                          rc3d-engine：仍偏少，需扩展
```

1. 操纵器以 **Action + 选区 + gizmo 库** 组合，与 Coin3D SoDragger 等效可渐进迁移到图节点。

2. **Viewport** 已从单一 surface 演进到 `ViewportLayout` + 多通道渲染。

3. **winit → 场景图**：`Event` + `HandleEventAction` + 全树键盘回调。
...
（表同前；Phase 0-1 实际已部分消化，余量主要在 Phase 2 引擎与 Phase 3 Backlog。）
